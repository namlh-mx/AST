...
public class OrgUnitDeclarationViewModelTests
{
    ...
    [Fact]
    public async Task BeginAddCommand_WhileLoadAsyncInFlight_SupersedesLoadAndKeepsBlankAddingForm()
    {
        var (vm, repo) = Build();
    ...
    // Card 276 / backlog 3.58: discard-confirmed overlay close must release the live-draft cache.
    [Fact]
    public async Task DiscardLiveSupplementalDraft_AfterDifferingLiveDraft_ClearsDirtyAndShowsSentence()
    {
        var (vm, repo) = Build();
        repo.ByIdentityResult = Dto(1, parentId: 5, Today.AddDays(-10), EffectivePeriod.OpenEnd, id: 77);
        await vm.LoadAsync(1, Today);
        ...
        vm.DiscardLiveSupplementalDraft();
        ...
        vm.Supplemental.Should().Be(new OrgUnitSupplementalDto(),
            "discard releases the live cache only — it must not rewrite committed Supplemental");
    }

    [Fact]
    public async Task DiscardLiveSupplementalDraft_WhenCardFieldAlreadyDirty_KeepsDirtyAndShowsNoSentence()
    {
        var (vm, repo) = Build();
        repo.ByIdentityResult = Dto(1, parentId: 5, Today.AddDays(-10), EffectivePeriod.OpenEnd, id: 77);
        await vm.LoadAsync(1, Today);
        ...
        vm.OrgNameFullVn = "Tên đã sửa trên thẻ";
        vm.IsDirty.Should().BeTrue();
        ...
        vm.IsDirty.Should().BeTrue(
            "discard must not wipe an unrelated card-field dirty with a lazy IsDirty = false");
        vm.HasUnsavedInput.Should().BeTrue();
        vm.StatusMessage.Should().NotBe(RevertToEntryStateMessage);
    }

    [Fact]
    public async Task DiscardLiveSupplementalDraft_WhileErrorOwnsBand_LeavesErrorAlone()
    {
