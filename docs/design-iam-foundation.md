...
- **File B — Root-admin break-glass (special, kept separate):**
  ...
  - ⭐ **The ROOT ORG UNIT is break-glass-only for every write.** Declaring one, adjusting one, closing or cancelling one, and replacing one are all reserved to a break-glass admin; an ordinary admin, however wide their data scope, may only READ it. Enforced in `OrgUnitDeclarationService` on all four paths — `OrgUnit.RootNotDeclarable` (Add), `OrgUnit.RootNotEditable` (Edit, which includes the "the unit ends on that date" gesture), `OrgUnit.RootNotClosable` (Close/Cancel) and `OrgUnit.RootNotReplaceable` (Replace) — and each permitted write records a second `audit_log` row (`orgunit-root-add-breakglass` / `orgunit-root-edit-breakglass` / `orgunit-root-close-breakglass` / `orgunit-root-replace-breakglass`) so that "a normally-forbidden operation was permitted" is a fact a security review can query for rather than infer from `parent_id`. ⚠️ **Replace is the one path with TWO sides.** `OrgUnit.RootNotReplaceable` fires when the *predecessor* is a root **or** when the *successor* is declared as a root, because the gesture writes both. The other three touch one unit each. **Consequence:** a fresh database has no root, so its first org unit can only be declared by a break-glass admin — which is what File B's own purpose above already says.
