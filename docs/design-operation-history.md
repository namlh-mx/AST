...
| `note` | VARCHAR(255) NULL | the note the operator gave **for the gesture** |

One index for the only query that reads it outside a join: `KEY idx_op_actor_time (username, occurred_at)`.

`operation` is application-wide — one table, not one per entity. It answers "which gesture?" and
nothing else.
...
| Column | Type | Meaning |
|---|---|---|
| `created_by_operation_id` | BIGINT UNSIGNED NOT NULL (after the contract phase, backlog 3.89) | the gesture that INSERTED this row |
...
field cannot carry that, and must not try to.
...
### 1.4 Header `note` versus row `reason`

On a no-new-row Cancel the operator's note has no row to live on.
...
The header never carries a per-row value, and a row's `reason` is never derived from the header.
...
*"If and only if" is load-bearing:* a successful transaction can write **no** version row —
`FunctionRepository.CreateAsync`'s `KeyAlreadyPresent` branch returns `Result.Success` from inside
the composite after the re-read under the lock finds the key. Minting at transaction entry would
commit an orphan header on exactly that path.
*Falsified by:* zero `operation` rows after a failed write; one header (not two) after a Role Save
that writes six rows; the losing concurrent-create caller leaves zero `operation` rows.

**OP2 — `operation` is insert-only.** No UPDATE, no DELETE, ever.
*Falsified by:* an `AST.Meta.Tests` guard asserting no `UPDATE`/`DELETE` statement in production
source names `operation`.
...
`isactive` back to `1` (the engine inserts a remnant instead of reactivating).
...
*Falsified by:* a per-site test over every write site listed in §4.1; a double-supersession test in
which the second attempt affects zero rows and leaves the first operation id intact; no row has
`isactive = 0` with `superseded_by_operation_id IS NULL`.

**OP5 — The history read is a UNION of two roles, not a GROUP BY on one column** (§3).
*Falsified by:* a real-MySQL test in which a Cancel with no predecessor, a Delete, and a Close each
produce exactly one history line — none missing, none doubled.

**OP6 — A system/batch actor mints one `operation` per atomic write, not per run** (§4.3).
*Falsified by:* a catalog-sync test where item 2 of 3 fails and the run leaves exactly one
`operation` row — the committed item — and no orphan header.
...
Each screen's history reads over its **aggregate**: the object's own version table plus the tables
that object owns.
...
| Function | `function_version` | **none — out of scope** |
| User | `user_version` | **none — out of scope** |

**Function and User have no history read, and this design does not give them one.** Neither
`function_version` nor `user_version` carries an `operation_kind` column (only `org_unit_version`,
`role_version` and `role_permission_version` do). Provenance columns (§1.2) still land on **all
five** tables — attribution is uniform; only the *read* is scoped.

Whoever builds those screens must first record `operation_kind` and settle a scope ruling (§3.3).
...
By the same rule `user → role`, `user → org_unit`, `grant → function` and `org_unit → parent` are
temporal dependencies, not ownership.
...
at runtime.
*Falsified by:* a real-MySQL test of this union exemplar (not review).
...
discriminator the projected rows collide.

**The ordering is three keys, not two.** Order by `occurred_at DESC, operation_id DESC`, then within
the operation by `source_table, version_id`.

Three consequences a naive `GROUP BY created_by_operation_id` gets wrong:

1. A gesture can appear **only** through the `superseded` half (a Cancel with no predecessor; a
   Delete). Grouping on `created_by_operation_id` alone omits it entirely.
...
3. Within one operation a row can play **both** roles. The union yields one line per
   ...
   The key is a **quadruple**: dropping `source_table` from it is the collision described above.
...
**The read is TWO stages, and the union is only the first.** The union is the provenance skeleton:
who, when, which rows, in which role. It deliberately carries **no business payload** — a `UNION`
demands one column shape across branches while `role_version` and `role_permission_version` show
different business columns.

**Stage 1** returns the skeleton, ordered. **Stage 2** fetches typed detail per table, keyed by the
`(source_table, version_id)` pairs stage 1 returned. Without stage 2 the screen renders opaque ids
and the current Role history grid regresses.

*Falsified by:* a Role Save that renames the role and grants a permission, whose single history line
expands to show both the name change and the grant, each with its own business columns.
...
**Org-unit reads are deliberately Global** — a product decision of 2026-08-04.
`LoadHistoryCoreAsync` passes `new DataScope(ScopeLevel.Global, …)` on purpose; cleaning up that call
site silently changes security behaviour. No live actor is protected by the predicate on the history
path today.

**Where losing the predicate would really bite:** `IsWithinScopeAsync` — the **write-path** gate —
shares the same SQL clause, deliberately factored so the two cannot diverge.

**Each module's history read keeps its own scope predicate**, and §3.2 describes the shape of the
...
*Falsified by:* at the **repository**, a non-Global `DataScope` for an identity outside the subtree
returns empty while historical descendants stay visible; at the **screen**, `LoadHistoryCoreAsync`
still passes `ScopeLevel.Global`. A repository test alone proves neither.
...
A per-object history grid exists so a person can **look a row up or search for it**. The filter
searches **only the criteria the grid already shows**.

A related unit is named by its **short name**, the internal-management name, the same string the tree
and the parent picker use. A grid row's **own** unit keeps both name columns; a **related** unit gets
one column and it is the short name.

This is a presentation rule. It does not license dropping a column that a temporal or authorization
claim depends on.
...
*"every state flip stamps it exactly once"* is broken by the very gesture that recorded it.
...
row's* provenance into a row the current gesture created — a silent lie.
...
- The parameter is **required, with no default** — so every production and test call site must supply
  it, and the *compiler* enforces the writer inventory (backlog 3.89). No grep-based sweep is trusted
  for this.
...
- **The context is TRANSACTION-AFFINE, and enforces it.** It holds the transaction it was built from,
  ...
  *Falsified by:* reusing a captured context in a second `ExecuteAsync` and asserting a clear
  failure.
  The context is the **sole mint authority**, so "who mints?" has one answer on both paths and no
  caller can mint a second row.
...
## Remaining slice: rulings and prerequisites
...
may reset data freely while the app is being built. The contract step needs no legacy provenance.
Any database still holding pre-migration version rows is **reset**, not backfilled. If a populated
database ever exists, this section is wrong and must be redesigned before the migration runs.

**Every phase that ships a migration carries three artifacts, in the same commit:**
...
**Raw-SQL fixtures bypass the compiler.** Each seeds a provenance id explicitly (a fixture-owned
`operation` row). An `AST.Meta.Tests` guard asserts that no raw `INSERT INTO {table}_version`
anywhere in the solution omits `created_by_operation_id`. Test fixtures are part of this inventory:
NOT NULL lands only after every fixture supplies a non-null operation.
