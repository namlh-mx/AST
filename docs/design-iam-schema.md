...
**Contract surface:** `IRoleRepository` is **read-only**, so role **version creation** is reachable only through `IRoleDeclarationService`. The narrow claim matters: `RoleRepository.CancelPlanAsync` and the inherited `CloseVersionAsync` remain public on the concrete class, so this is not "all role writes go through the service". `IRolePermissionRepository` keeps its plain `UpsertAsync` but declares no `CreateIdentityAsync`/`DeleteEmptyIdentityAsync`: grant identities are minted inside the composite transaction that writes their first version (`design-effective-period.md` §7), so the interface can neither produce nor need to compensate a zero-version header. `IOrgUnitRepository` declares no writer. Org-unit Add, Edit, Replace, Close and Cancel go through `IOrgUnitDeclarationService`, which derives the `VersionOperationKind` server-side, and there is no Delete business operation. `AST.Meta.Tests/OrgUnitWritePathAbsenceTests` guards that no production use of an org-unit version writer, resolved by member and type, sits outside `OrgUnitDeclarationService`, `OrgUnitRepository` and `VersionedRepository<TVersion>`. The asymmetry is deliberate: Role Save owns the aggregate/admin-flag-lock behaviour that makes an unlocked plain writer hazardous, so a business-layer caller depends on the use-case contract `IRoleDeclarationService`, whose `internal` implementation depends on the CONCRETE repository classes.
